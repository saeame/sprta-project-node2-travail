...
class UserService {
    ...
    signup = async ({ email, password, confirm, phone, address, name }) => {
        try {
            ...
            const {
                dataValues: { userId, name: userName },
            ...
            await addressRepository.createthisAddress(userId, address, "기본주소", userName);
        } catch (err) {
            throw err;
        }
    };
    ...
    updateUser = async ({ email, password, checkPass, address, phone }, userData) => {
        try {
