...
// const authMiddlewares  -> 로그인여부 확인해서 페이지 렌더링 또는 리다이렉트 여부 결정
...
router.get("/product/:productId", (req, res) => {
    res.render("product.ejs", {components: "currentProduct"});
});
...
router.get("/product", (req, res) => {
    res.render("product.ejs", {components: "product"});
});
...
// 상품관리 (admin only)
router.get("/product/admin", authMiddlewares, (req, res) => {
    // 로그인이 안됐을 경우?
    if (!res.locals.user) {
        return res.render("login.ejs", {
            components: "login",
